...
from LLCB_test_pt_act_simple import Add_LoRA, Model_init, preprocess, input_processing_real_batch, parse_args
import time
...
class TrainTester(BaseTrainTester):
    ...
    #增加了输入参数embedding表示act token的last hidden layer向量，lmcliploss表示lmloss和cliploss之和，二者分别为整个loss组成的前两项
    def train_one_step(self, model, criterion, optimizer, step_id, sample, embedding, lmcliploss=None, act_pred=None):
        ...
        if lmcliploss is not None:
            ...
            loss = action_loss
        # time2_5 = time.time() #timer
        # print("时间段1.5:", time2_5-time2) #timer
        loss.backward()
        # time3 = time.time() #timer
        # print("时间段二:", time3-time2) #timer
        # Update
...
def traj_collate_fn(batch):
    ...
    # ret_dict["instr_text"] = [item['instr_text'] for item in batch]  # 单独处理 instr_text
    ret_dict["instr_text"] = []
    ret_dict["task"] = []
    for item in batch:
        # print("item内容为",)
        # print("一个item中curr_gripper为", item['curr_gripper'])
        # print("一个item中curr_gripper长度为", item['curr_gripper'].shape)
        ret_dict["instr_text"] += item['instr_text']
        ret_dict["task"] += item['task']
    # print("task长度",len(ret_dict["task"]))
    return ret_dict
