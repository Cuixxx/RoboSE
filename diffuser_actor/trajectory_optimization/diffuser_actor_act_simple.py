...
class DiffuserActorACTS(nn.Module):
    ...
    def forward(
        ...
        for layer_pred in pred:
            ...
            loss = (
                30 * (F.l1_loss(trans, noise[..., :3], reduction='mean')+0.2*F.l1_loss(trans_llm, gt_trajectory[start_idx, 0, :3], reduction='mean'))
                + 10 * (F.l1_loss(rot, noise[..., 3:9], reduction='mean')+0.2*F.l1_loss(rot_llm, gt_trajectory[start_idx, 0, 3:9], reduction='mean'))
            )
