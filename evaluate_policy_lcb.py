...
def evaluate_policy(model, LLM_model, clip_image_processor, env, conf_dir, eval_log_dir=None, save_video=False,
                    sequence_indices=[]):
    """
    ...
    for seq_ind, (initial_state, eval_sequence) in enumerate(eval_sequences):
        ...
        result, videos = evaluate_sequence(
            env, model, LLM_model, clip_image_processor, task_oracle, initial_state,
...
def evaluate_sequence(env, model, LLM_model, clip_image_processor, task_checker, initial_state, eval_sequence,
    ...
    for subtask in eval_sequence:
        ...
        success, video = rollout(env, model, LLM_model, clip_image_processor, task_checker,
...
def rollout(env, model, LLM_model, clip_image_processor, task_oracle, subtask, lang_annotation):#这是运行试验的主要代码
    """
    ...
    for step in pbar:
        ...
        conversations, questions = transfer(lang_annotation)
        ...
        image_clip, image, input_ids, attention_masks, targets = input_processing_real_batch(image_tensor=obs["rgb_obs"]["rgb_static"], conv_list=conversations, clip_image_processor= clip_image_processor)
...
def main(args):
    ...
    # 加载权重文件的状态字典
    state_dict = torch.load('/zhaohan/Wenxuan/3d_diffuser_actor/train_logs/Planner_Calvin/10022208lmpart-gpu1-accumulation-val_frequency5-step165700-step2105000-C192-B90-lr3e-4-DI1-20-H1-DT25-backboneclip-S256,256-R1-wd5e-3/0065019/pytorch_model.bin', map_location='cpu')
    # 创建一个新的状态字典，修改参数名称
    new_state_dict = {}
    ...
    #加载lora部分权重
    peft_model = PeftModel.from_pretrained(LCB_model, '/zhaohan/Wenxuan/3d_diffuser_actor/train_logs/Planner_Calvin/10022208lmpart-gpu1-accumulation-val_frequency5-step165700-step2105000-C192-B90-lr3e-4-DI1-20-H1-DT25-backboneclip-S256,256-R1-wd5e-3/0065019')   
    ...
    evaluate_policy(model, LLM_model, env, clip_image_processor,
...
def input_processing_real_batch(image_tensor, conv_list, clip_image_processor):
    '''
