...
from diffuser_actor import DiffuserActorACT
    ...
    def get_text_encoder(self):
        def load_model(encoder) -> transformers.PreTrainedModel:
            if encoder == "bert":
                ...
                model = transformers.CLIPTextModel.from_pretrained("/storage/dingpengxiang/clip-vit-large-patch14")
            else:
        ...
        def load_tokenizer(encoder) -> transformers.PreTrainedTokenizer:
            if encoder == "bert":
                ...
                tokenizer = transformers.CLIPTokenizer.from_pretrained(
                    "/storage/dingpengxiang/clip-vit-large-patch14"
                )
            else:
                raise ValueError(f"Unexpected encoder {encoder}")
